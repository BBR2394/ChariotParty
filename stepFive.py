...
# @Last Modified time: 2019-09-20 16:15:53
...
class Case:
	...
	def effet(self, joueur):
		if self.couleur == 1: #green +3
			joueur.setChar(joueur.getChar() + 3)
		if self.couleur == 2: #red -3
			temp_char = joueur.getChar() - 3
			if temp_char < 0:
				joueur.setChar(0)
			else:
				joueur.setChar(temp_char)
		#if self._color = 3: #blue do nothing so ..
...
class Plateau:
	...
	def gen_gold(self, min):
		return randint(min, self.number_case - 1)

	def initGold(self):
		self.pos_gold = self.gen_gold(0)
	...
	def addCase(self):
		a_case = Case(self.gen_case())
		self.lstCase.append(a_case)
		#self.lstCaseInt.append(a_case.getType())
